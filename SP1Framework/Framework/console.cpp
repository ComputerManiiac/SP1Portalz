...
void Console::initConsole(COORD consoleSize, LPCSTR lpConsoleTitle)
{
	...
	// CUSTOM: Disable Markup Mode to prevent Syntax Highlighting
...
void Console::toggleCursor() {
	...
	cursorInfo.dwSize = 1;
	cursorInfo.bVisible = consoleCursor;
	SetConsoleCursorInfo(m_hScreenBuffer, &cursorInfo);
}
...
COORD Console::getMouseXY() {
	...
	GetConsoleScreenBufferInfo(m_hScreenBuffer, &inf);
